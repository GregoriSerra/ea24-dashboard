...
// Gráficos Highcharts (referencias para actualizar datos sin recrearlos cada vez)
let chartPrimerGrafico, chartSegundoGrafico, chartTercerGrafico;

// Función para cargar el primer gráfico (medias más altas por género)
...
// Función para cargar el segundo gráfico (jugadores por liga en un gráfico donut)
function cargarSegundoGrafico() {
    fetch('http://localhost/backend/data.php?tipo=ligas')
        .then(response => response.json())
        .then(datos => {
            if (!Array.isArray(datos)) {
                console.error('Respuesta inesperada del servidor:', datos);
            ...
            if (!chartSegundoGrafico) {
                ...
                // Actualizamos los datos del gráficosi ya existe
                chartSegundoGrafico.series[0].setData(dataLigas);
            }
        })
        .catch(error => console.error('Error cargando datos de ligas:', error));
}

// Función para cargar el tercer gráfico(temp actual estadios)
    ...
    function cargarEstadio(index) {
        if (index >= estadios.length) {
            if (!chartTercerGrafico) {
                ...
                // Actualizamos los datos del gráfico si ya existe
...
// Inicia la carga inicial y configura el refresco periódico cada 15 segundo.
